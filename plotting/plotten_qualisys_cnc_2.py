...
file_list = fd.askopenfilename(title='Qualisys-Daten auswählen',initialdir='D:\\FH Aachen\\Lernen\\Abschlussarbeiten\\Praxisprojekt\\Endergebnisse\\',multiple=True,filetypes=(('Ergebnissdateien', '*.txt'), ('All files', '*.*')))
...
for file in file_list:
    ...
    ax_2.set(xlabel='x [mm]', ylabel='y [mm]', title='G-Code und Qualisys')
    ax_2.grid()
    ax_2.legend()
    ...
    ax_3.set(xlabel='x', ylabel='y', title='G-Code und Qualisys')
    ax_3.grid()
    ax_3.legend()
    ...
    #print("zeitpunkte", len(zeitpunkte), zeitpunkte)
    #plt.show()
    max_list.append(max(dist_gesamt))
    all_list = all_list + dist_gesamt
print("maxlist",max_list)
print("maxwert: ", max(max_list), "durchschnitt: ", mean(max_list))

bins = np.linspace(0, 0.3, 200)
fig, ax = plt.subplots()
ax.hist(all_list,bins, align='mid', ec='black')
ax.set(xlabel='Abweichung [mm]', ylabel='Häufigkeit')
print("stdabw: ", np.std(all_list), mean(all_list))
plt.show()
