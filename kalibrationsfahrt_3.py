...
def koord_trans(xwerte_q, ywerte_q, zwerte_q, freq):
    ...
    print('drehwinkel_z',drehwinkel_z)
    ...
    return(drehmatrix, drehmatrix_z, xoff_dreh, yoff_dreh, zoff_dreh)
...
def datei_schreiben(dateiname):
    ...
    matrix_ebene, matrix_z, xoff, yoff, zoff= koord_trans(xwerte_q, ywerte_q, zwerte_q, freq)
...
if __name__ == "__main__":
    ...
    dateiname = "ergebnisse/" + time.strftime("%Y.%m.%d_%H.%M",time.gmtime()) +"_kalibration.txt"
    qualisys_task = Qualisys_stream(dateiname)
